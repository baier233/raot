...
namespace utils::animation {
	...
	public:
		...
		inline static func EASE_IN_EXPO() {
			return [](double x) { return x == 0 ? 0 : std::pow(2, 10 * x - 10); };
		}

		inline static func EASE_OUT_EXPO() {
			return [](double x) { return x == 1 ? 1 : 1 - std::pow(2, -10 * x); };
		}

		inline static func EASE_IN_OUT_EXPO() {
		...
		inline static func EASE_OUT_BACK() {
			return [](double x) {
				double inertia = 1.5;
		...
		inline static func EASE_IN_OUT_BACK() {
			return [](double x) {
				double inertia = 1.5;
	...
	public:
		inline animation_obj(long duration, easing::func easing)
			: duration(duration), easing(easing) {}

		inline void set_target(float targetValue) {
		...
		inline long get_duration() const { return duration; }
		inline void set_duration(long newDuration) { duration = newDuration; }

		inline easing::func get_easing() const { return easing; }

		inline bool is_finished() const { return finished; }
		inline void set_finished(bool isFinished) { finished = isFinished; }
		inline void set_easing(easing::func newEasing) { easing = newEasing; }

		inline float get_current() const { return current; }
		inline float get_target() const { return target; }
