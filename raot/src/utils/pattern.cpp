...
#include <iostream>
uintptr_t utils::pattern::find(uintptr_t address, const char* pattern)
{
...
uintptr_t utils::pattern::find(const char* module, const char* pattern)
{
	uintptr_t moduleAdressmm = 0;
	//moduleAdressmm = *(uintptr_t*)(__readgsqword(0x60) + 0x10);
...
uintptr_t utils::pattern::get_absolute_addr(uintptr_t address, int offset, int size)
{
	if (address)
	{
