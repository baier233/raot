...
#pragma clang diagnostic ignored "-Wenum-enum-conversion"           // warning: bitwise operation between different enumeration type ('XXXFlags_' and 'XXXFlagsPrivate_')
#pragma clang diagnostic ignored "-Wdeprecated-enum-enum-conversion"// warning: bitwise operation between different enumeration type ('XXXFlags_' and 'XXXFlagsPrivate_') is deprecated
...
#pragma GCC diagnostic ignored "-Wdeprecated-enum-enum-conversion"  // warning: bitwise operation between different enumeration type ('XXXFlags_' and 'XXXFlagsPrivate_') is deprecated
#endif
...
bool ImGui::DataTypeApplyFromText(const char* buf, ImGuiDataType data_type, void* p_data, const char* format, void* p_data_when_empty)
{
	...
	// Small type need a 32-bit buffer to receive the result from scanf()
	int v32 = 0;
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
	...
	if (*v != v_cur && is_bounded)
	{
		...
		{
			// Clamp values + handle overflow/wrap-around for integer type.
...
// For 32-bit and larger type, slider bounds are limited to half the natural type range.
...
{
	// Printf/scanf type modifiers: I/L/h/j/l/t/w/z. Other uppercase letters qualify as type aka end of the format.
	if (fmt[0] != '%')
		return fmt;
...
// - For scanning we need to remove all width and precision fields and flags "%+3.7f" -> "%f". BUT don't strip type like "%I64d" which includes digits. ! "%07I64d" -> "%I64d"
