...
#endif
// - ImMin/ImMax/ImClamp/ImLerp/ImSwap are used by widgets which support variety of type: signed/unsigned int/long long float/double
// (Exceptionally using templates here but we could also redefine them for those type)
...
struct ImGuiContext
{
	...
	ImGuiDataTypeStorage    DataTypeZeroValue;                  // 0 for all data type
	int                     BeginMenuDepth;
	int                     BeginComboDepth;
...
namespace ImGui
{
	...
	// Template functions are instantiated in imgui_widgets.cpp for a finite number of type.
