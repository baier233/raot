...
Index of this file:
// [SECTION] Header mess
// [SECTION] Forward declarations and basic type
...
// [SECTION] Obsolete functions and type
...
struct ImVec2
{
	...
	IM_VEC2_CLASS_EXTRA     // Define additional constructors and implicit cast operators in imconfig.h to convert back and forth between your math type and ImVec2.
#endif
};
...
struct ImVec4
{
	...
	IM_VEC4_CLASS_EXTRA     // Define additional constructors and implicit cast operators in imconfig.h to convert back and forth between your math type and ImVec4.
#endif
};
...
namespace ImGui
{
	...
	// - Those are merely shortcut to calling Text() with a format string. Output single value in "name: value" format (tip: freely declare more in your code to handle your type. you can add functions to the ImGui namespace)
	...
	IMGUI_API bool          SetDragDropPayload(const char* type, const void* data, size_t sz, ImGuiCond cond = 0);  // type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal type. Data is copied and held by imgui. Return true when payload has been accepted.
...
enum ImGuiInputTextFlags_
{
	...
	ImGuiInputTextFlags_CallbackResize = 1 << 21,  // Callback on buffer capacity changes request (beyond 'buf_size' parameter value), allowing the string to grow. Notify when the string wants to be resized (for string type which hold a cache of their Size). You will be provided a new BufSize in the callback and NEED to honor it. (see misc/cpp/imgui_stdlib.h for an example of using this)
...
// Standard Drag and Drop payload type. You can define you own payload type using short strings. Types starting with '_' are defined by Dear ImGui.
...
// The keyboard key enum values are named after the keys on a standard US keyboard, and on other keyboard type the keys reported may not match the keycaps.
enum ImGuiKey : int
{
	// Keyboard
	ImGuiKey_None = 0,
...
struct ImGuiIO
{
	...
	bool        ConfigInputTrickleEventQueue;   // = true           // Enable input queue trickling: some type of events submitted during the same frame (e.g. button down + up) will be spread over multiple frames, improving interactions with low framerates.
...
// Types are NOT stored, so it is up to you to make sure your Key don't collide with different type.
struct ImGuiStorage
{
	// [Internal]
...
// - This is in order to allow user enabling implicit cast operators between ImVec2/ImVec4 and their own type (using IM_VEC2_CLASS_EXTRA in imconfig.h)
// - Add '#define IMGUI_DEFINE_MATH_OPERATORS' before including this file (or in imconfig.h) to access courtesy maths operators for ImVec2 and ImVec4.
