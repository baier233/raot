...
namespace kiero {
	...
	auto Init(RenderType::Enum renderType) -> Status::Enum;
	auto Shutdown() -> void;

	auto Bind(uint16_t index, void* function) -> Status::Enum;
	auto Unbind(uint16_t index) -> void;

	auto GetRenderType() -> RenderType::Enum;
	auto GetMethodsTable() -> uint150_t*;
}
